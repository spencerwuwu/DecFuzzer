//
// This file was generated by the Retargetable Decompiler
// Website: https://retdec.com
//

#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

// ------------------- Function Prototypes --------------------

int32_t __x86_get_pc_thunk_bx(int32_t a1);
int32_t __x86_get_pc_thunk_dx(int32_t a1);
int32_t _fini(void);
int32_t _init(void);
int32_t _start(int32_t a1);
void crc32_8bytes(int32_t val);
void crc32_byte(char b);
void crc32_gentab(void);
int16_t func_1(void);
int32_t function_1040(char * s1, char * s2);
int32_t function_1050(int32_t main2, int32_t argc, char ** ubp_av, void (*init)(), void (*fini)(), void (*rtld_fini)());
int32_t function_1060(char * format, ...);
int32_t function_109c(int32_t a1);
int32_t function_10b0(void);
int32_t function_10f0(void);
int32_t function_1140(void);
int32_t function_1190(void);
void platform_main_begin(void);
void platform_main_end(int32_t crc, int32_t flag);
int32_t set_var(int32_t g_12_l, int32_t g_14_l, int16_t g_23_l, int32_t g_47_l, int32_t g_49_l, int32_t g_50_l, int32_t g_53_l, int16_t g_68_l, char g_69_l, char g_70_l, char g_94_l, int16_t g_95_l, int32_t g_97_l, int32_t g_98_l, int16_t g_99_l, char g_100_l, int32_t g_105_l, int32_t g_128_l, char g_206_l);
void transparent_crc(int32_t val, char * vname, int32_t flag);

// --------------------- Global Variables ---------------------

int32_t g1 = 124; // 0x30b8
int32_t g2 = 392; // 0x3118
int32_t g3;

// ------- Dynamically Linked Functions Without Header --------

int32_t __libc_start_main(int32_t a1, int32_t a2, char ** a3, void (*a4)(), void (*a5)(), void (*a6)());

// ------------------------ Functions -------------------------

// Address range: 0x1000 - 0x1024
int32_t _init(void) {
    // 0x1000
    int32_t v1; // 0x1000
    __x86_get_pc_thunk_bx(v1);
    return *(int32_t *)(v1 + 0x3fdb);
}

// Address range: 0x1040 - 0x1046
int32_t function_1040(char * s1, char * s2) {
    // 0x1040
    return strcmp(s1, s2);
}

// Address range: 0x1050 - 0x1056
int32_t function_1050(int32_t main2, int32_t argc, char ** ubp_av, void (*init)(), void (*fini)(), void (*rtld_fini)()) {
    // 0x1050
    return __libc_start_main(main2, argc, ubp_av, init, fini, rtld_fini);
}

// Address range: 0x1060 - 0x1066
int32_t function_1060(char * format, ...) {
    // 0x1060
    return printf(format);
}

// Address range: 0x1070 - 0x109c
int32_t _start(int32_t a1) {
    // 0x1070
    int32_t v1; // 0x1070
    function_109c(v1);
    int32_t v2 = *(int32_t *)(v1 + 0x3f68); // 0x1090
    int32_t v3; // 0x1070
    __libc_start_main(v2, a1, (char **)&v3, NULL, NULL, (void (*)())&g3);
    __asm_hlt();
    // UNREACHABLE
}

// Address range: 0x109c - 0x10a0
int32_t function_109c(int32_t a1) {
    // 0x109c
    int32_t result; // 0x109c
    return result;
}

// Address range: 0x10a0 - 0x10a4
int32_t __x86_get_pc_thunk_bx(int32_t a1) {
    // 0x10a0
    int32_t result; // 0x10a0
    return result;
}

// Address range: 0x10b0 - 0x10e9
int32_t function_10b0(void) {
    // 0x10b0
    __x86_get_pc_thunk_dx((int32_t)&g3);
    int32_t v1; // 0x10b0
    return v1 + 0x3f93;
}

// Address range: 0x10f0 - 0x1137
int32_t function_10f0(void) {
    // 0x10f0
    __x86_get_pc_thunk_dx((int32_t)&g3);
    return 0;
}

// Address range: 0x1140 - 0x118c
int32_t function_1140(void) {
    // 0x1140
    int32_t v1; // 0x1140
    int32_t v2 = __x86_get_pc_thunk_bx(v1); // 0x1148
    char * v3 = (char *)(v1 + 0x3efb); // 0x1156
    int32_t result = v2; // 0x115d
    if (*v3 == 0) {
        // 0x115f
        result = function_10b0();
        *v3 = 1;
    }
    // 0x1187
    return result;
}

// Address range: 0x1190 - 0x1199
int32_t function_1190(void) {
    // 0x1190
    return function_10f0();
}

// Address range: 0x1199 - 0x119d
int32_t __x86_get_pc_thunk_dx(int32_t a1) {
    // 0x1199
    int32_t result; // 0x1199
    return result;
}

// From module:   /home/weicheng/Documents/CS699_WASM/DecFuzzer/seed_all/s793r2.c
// Address range: 0x11a0 - 0x14cd
// Line range:    240 - 267
int main(int argc, char ** argv) {
    int32_t v1 = 0; // 0x11ce
    int32_t v2; // 0x11a0
    if (argv == (char **)2) {
        int32_t str = *(int32_t *)(v2 + 4); // 0x11da
        v1 = strcmp((char *)str, (char *)(v2 + 0x1e5c)) == 0;
    }
    // 0x11ff
    platform_main_begin();
    crc32_gentab();
    func_1();
    int32_t v3 = *(int32_t *)(v2 + 0x3e68); // 0x121a
    transparent_crc(v3, (char *)(v2 + 0x1e5e), v1);
    int32_t v4 = *(int32_t *)(v2 + 0x3ea0); // 0x123c
    transparent_crc(v4, (char *)(v2 + 0x1e63), v1);
    uint16_t v5 = *(int16_t *)(v2 + 0x3e6c); // 0x125e
    transparent_crc((int32_t)v5, (char *)(v2 + 0x1e68), v1);
    int32_t v6 = *(int32_t *)(v2 + 0x3e70); // 0x1281
    transparent_crc(v6, (char *)(v2 + 0x1e6d), v1);
    int32_t v7 = *(int32_t *)(v2 + 0x3e74); // 0x12a3
    transparent_crc(v7, (char *)(v2 + 0x1e72), v1);
    int32_t v8 = *(int32_t *)(v2 + 0x3e78); // 0x12c5
    transparent_crc(v8, (char *)(v2 + 0x1e77), v1);
    int32_t v9 = *(int32_t *)(v2 + 0x3ea4); // 0x12e7
    transparent_crc(v9, (char *)(v2 + 0x1e7c), v1);
    int16_t v10 = *(int16_t *)(v2 + 0x3e7c); // 0x1309
    transparent_crc((int32_t)v10, (char *)(v2 + 0x1e81), v1);
    unsigned char v11 = *(char *)(v2 + 0x3e7e); // 0x132c
    transparent_crc((int32_t)v11, (char *)(v2 + 0x1e86), v1);
    unsigned char v12 = *(char *)(v2 + 0x3e7f); // 0x134f
    transparent_crc((int32_t)v12, (char *)(v2 + 0x1e8b), v1);
    char v13 = *(char *)(v2 + 0x3ea8); // 0x1372
    transparent_crc((int32_t)v13, (char *)(v2 + 0x1e90), v1);
    int16_t v14 = *(int16_t *)(v2 + 0x3e80); // 0x1395
    transparent_crc((int32_t)v14, (char *)(v2 + 0x1e95), v1);
    int32_t v15 = *(int32_t *)(v2 + 0x3e84); // 0x13b8
    transparent_crc(v15, (char *)(v2 + 0x1e9a), v1);
    int32_t v16 = *(int32_t *)(v2 + 0x3e88); // 0x13da
    transparent_crc(v16, (char *)(v2 + 0x1e9f), v1);
    int16_t v17 = *(int16_t *)(v2 + 0x3e8c); // 0x13fc
    transparent_crc((int32_t)v17, (char *)(v2 + 0x1ea4), v1);
    unsigned char v18 = *(char *)(v2 + 0x3e8e); // 0x141f
    transparent_crc((int32_t)v18, (char *)(v2 + 0x1ea9), v1);
    int32_t v19 = *(int32_t *)(v2 + 0x3e90); // 0x1444
    transparent_crc(v19, (char *)(v2 + 0x1eaf), v1);
    int32_t v20 = *(int32_t *)(v2 + 0x3eac); // 0x1466
    transparent_crc(v20, (char *)(v2 + 0x1eb5), v1);
    unsigned char v21 = *(char *)(v2 + 0x3e94); // 0x1488
    transparent_crc((int32_t)v21, (char *)(v2 + 0x1ebb), v1);
    platform_main_end(-1 - *(int32_t *)(v2 + 0x3e98), v1);
    return 0;
}

// From module:   /home/weicheng/Documents/CS699_WASM/DecFuzzer/./seed_all/stdint-intn.h
// Address range: 0x14d0 - 0x14d5
// Line range:    43 - 46
void platform_main_begin(void) {
    // 0x14d0
    return;
}

// From module:   /home/weicheng/Documents/CS699_WASM/DecFuzzer/./seed_all/stdint-uintn.h
// Address range: 0x14e0 - 0x1588
// Line range:    58 - 75
void crc32_gentab(void) {
    for (int32_t i = 0; i < 256; i++) {
        int32_t v1 = 8; // 0x155a
        uint32_t v2 = i / 2;
        int32_t v3 = i % 2 == 0 ? v2 : v2 ^ -0x12477ce0;
        v1--;
        int32_t v4 = v3; // 0x1521
        while (v1 != 0) {
            // 0x1527
            v2 = v4 / 2;
            v3 = v4 % 2 == 0 ? v2 : v2 ^ -0x12477ce0;
            v1--;
            v4 = v3;
        }
        // 0x1565
        int32_t v5; // 0x14e0
        *(int32_t *)(v5 + 0x3b71 + 4 * i) = v3;
    }
}

// From module:   /home/weicheng/Documents/CS699_WASM/DecFuzzer/seed_all/s793r2.c
// Address range: 0x1590 - 0x1ec9
// Line range:    56 - 232
int16_t func_1(void) {
    // 0x1590
    set_var(-0x15912527, 0, -0x2cc3, -5, 0xf6bb026, -1, 0, -0x593a, -31, 1, 0, 1, 0x1cd86053, 0x584dd3ea, 0x390a, 21, 4, 0, -75);
    return 4;
}

// From module:   /home/weicheng/Documents/CS699_WASM/DecFuzzer/./seed_all/stdint-uintn.h
// Address range: 0x1ed0 - 0x1f2f
// Line range:    95 - 101
void transparent_crc(int32_t val, char * vname, int32_t flag) {
    // 0x1ed0
    crc32_8bytes((int32_t)vname);
    int32_t v1; // 0x1ed0
    if (v1 != 0) {
        // 0x1f04
        printf((char *)(v1 + (int32_t)&g2 - 0x1f87));
    }
}

// From module:   /home/weicheng/Documents/CS699_WASM/DecFuzzer/./seed_all/stdint-intn.h
// Address range: 0x1f30 - 0x1f64
// Line range:    49 - 120
void platform_main_end(int32_t crc, int32_t flag) {
    // 0x1f30
    int32_t v1; // 0x1f30
    printf((char *)(v1 + (int32_t)&g1 - 0x1f64));
}

// From module:   /home/weicheng/Documents/CS699_WASM/DecFuzzer/seed_all/s793r2.c
// Address range: 0x1f70 - 0x2073
// Line range:    46 - 48
int32_t set_var(int32_t g_12_l, int32_t g_14_l, int16_t g_23_l, int32_t g_47_l, int32_t g_49_l, int32_t g_50_l, int32_t g_53_l, int16_t g_68_l, char g_69_l, char g_70_l, char g_94_l, int16_t g_95_l, int32_t g_97_l, int32_t g_98_l, int16_t g_99_l, char g_100_l, int32_t g_105_l, int32_t g_128_l, char g_206_l) {
    // 0x1f70
    int32_t v1; // 0x1f70
    *(int32_t *)(v1 + (int32_t)"er hashing %s : %X\n" + 32) = g_14_l;
    *(int32_t *)(v1 + (int32_t)"er hashing %s : %X\n" + 88) = (int32_t)g_23_l;
    *(int16_t *)(v1 + (int32_t)"er hashing %s : %X\n" + 36) = (int16_t)g_47_l;
    *(int32_t *)(v1 + (int32_t)"er hashing %s : %X\n" + 40) = g_49_l;
    *(int32_t *)(v1 + (int32_t)"er hashing %s : %X\n" + 44) = g_50_l;
    *(int32_t *)(v1 + (int32_t)"er hashing %s : %X\n" + 48) = g_53_l;
    *(int32_t *)(v1 + (int32_t)"er hashing %s : %X\n" + 92) = (int32_t)g_68_l;
    *(int16_t *)(v1 + (int32_t)"er hashing %s : %X\n" + 52) = (int16_t)g_69_l;
    *(char *)(v1 + (int32_t)"er hashing %s : %X\n" + 54) = g_70_l;
    *(char *)(v1 + (int32_t)"er hashing %s : %X\n" + 55) = g_94_l;
    *(char *)(v1 + (int32_t)"er hashing %s : %X\n" + 96) = (char)g_95_l;
    *(int16_t *)(v1 + (int32_t)"er hashing %s : %X\n" + 56) = (int16_t)g_97_l;
    *(int32_t *)(v1 + (int32_t)"er hashing %s : %X\n" + 60) = g_98_l;
    *(int32_t *)(v1 + (int32_t)"er hashing %s : %X\n" + 64) = (int32_t)g_99_l;
    *(int16_t *)(v1 + (int32_t)"er hashing %s : %X\n" + 68) = (int16_t)g_100_l;
    *(char *)(v1 + (int32_t)"er hashing %s : %X\n" + 70) = (char)g_105_l;
    *(int32_t *)(v1 + (int32_t)"er hashing %s : %X\n" + 72) = g_128_l;
    *(int32_t *)(v1 + (int32_t)"er hashing %s : %X\n" + 100) = (int32_t)g_206_l;
    return 0;
}

// From module:   /home/weicheng/Documents/CS699_WASM/DecFuzzer/./seed_all/stdint-uintn.h
// Address range: 0x2080 - 0x2100
// Line range:    86 - 92
void crc32_8bytes(int32_t val) {
    // 0x2080
    int32_t v1; // 0x2080
    uint32_t v2 = v1;
    crc32_byte((char)v2);
    crc32_byte((char)(v2 / 256));
    crc32_byte((char)(v2 / 0x10000));
    crc32_byte((char)(v2 / 0x1000000));
}

// From module:   /home/weicheng/Documents/CS699_WASM/DecFuzzer/./seed_all/stdint-uintn.h
// Address range: 0x2100 - 0x2144
// Line range:    78 - 82
void crc32_byte(char b) {
    // 0x2100
    int32_t v1; // 0x2100
    int32_t * v2 = (int32_t *)(v1 + 0x2f3b); // 0x2113
    uint32_t v3 = *v2; // 0x2113
    char v4; // 0x2100
    *v2 = v3 / 256 ^ *(int32_t *)(v1 + 0x2f53 + 4 * (v3 % 256 ^ (int32_t)v4));
}

// Address range: 0x2144 - 0x215c
int32_t _fini(void) {
    // 0x2144
    int32_t v1; // 0x2144
    return __x86_get_pc_thunk_bx(v1);
}

// --------------------- Meta-Information ---------------------

// Detected compiler/packer: llvm (15.0.7)
// Detected functions: 22

